...
class UserRegistrationService:  # pylint: disable=R0903
    ...
    def register_user(user_data):  # pylint: disable=C0116
        ...
        hashed_password = PasswordService.hash_password(user_data.get("password"))
...
class UserLoginService:  # pylint: disable=R0903
    ...
    def login_user(email, plaintext_password):  # pylint: disable=C0116
        ...
        is_password_matched = PasswordService.check_password(
            plaintext_password, user_hashed_password
        )
...
class PasswordService:  # pylint: disable=R0903
    ...
    def hash_password(password):  # pylint: disable=C0116
        return hashpw(password.encode("utf-8"), gensalt(16)).decode("utf-8")
...
class OTPService:  # pylint: disable=R0903
    ...
    def generate_otp(email):  # pylint: disable=C0116
        ...
        otp_template = render_template(
            "auth/one-time-password.html", otp=seven_digit_otp, user_name=email
        )
    ...
    def generate_jwt_csrf_token(email, user_id):  # pylint: disable=C0116
        ...
        with current_app.app_context():
            ...
            csrf_token = get_csrf_token(
                access_token
            )  # This returns boolean instead of bytes-like object or a string
...
class ForgotPasswordService:
    ...
    @staticmethod
    def send_forgot_password_link(email):  # pylint: disable=C0116
        front_end_forgot_password_url = (
            getenv("LOCAL_FRONTEND_URL", "") + "/reset-password/"
        )
    ...
    @staticmethod
    def verify_forgot_password_token(token, new_password: str):  # pylint: disable=C0116
        new_hashed_password = hashpw(new_password.encode("utf-8"), gensalt(16)).decode(
            "utf-8"
        )
...
class EmailVerificationService:
    ...
    @staticmethod
    def resend_email_verification(email):  # pylint: disable=C0116
        verification_token = base64.b64encode(urandom(24)).decode("utf-8")
        ...
        verification_url = (
            ...
            + getenv("API_VERIFY_EMAIL", "")
            + verification_token
        )
        verification_template = render_template(
            "auth/welcome.html", verification_url=verification_url, user_name=email
        )
        SendMailService.send_mail(
