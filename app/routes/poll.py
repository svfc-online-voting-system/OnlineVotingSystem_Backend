...
def add_poll():
    ...
    new_poll_voting_data["created_by"] = get_jwt_identity().get("user_id")  # type: ignore
...
def add_option():
    ...
    poll_service = PollService()
    poll_service.add_option(
        poll_id=poll_id, user_id=get_jwt_identity(), option_text=option_text
    )
    return set_response(
